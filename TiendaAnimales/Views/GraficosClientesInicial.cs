...
{
    public partial class GraficosClientesInicial : Form
    {
        public GraficosClientesInicial()
        {
            InitializeComponent();
        }
        ...
        private void GraficosClientesInicial_Load(object sender, EventArgs e)
        {
            try
            {
                string rutaXml = Path.Combine(Application.StartupPath, "Data", "DatosTienda.xml");
                string rutaInforme = Path.Combine(Application.StartupPath, "Reports", "reporteGraficoClientesInicial.rdlc");
                ...
                var resumen = datos.Clientes
                    .GroupBy(c => string.IsNullOrWhiteSpace(c.Nombre) ? "?" : c.Nombre.Substring(0, 1).ToUpper())
